 <!--
  The main student dashboard where everything is displayed
  Uses real time Firebase listeners so everything updates instantly without refreshing
  Shows the student's profile, quiz score, stats the points, hours, tasks
  overall progress bar, enrolled tasks with evidence submission
  and certificate download when all tasks are approved
-->
...
<template>
  <div class="mx-auto max-w-5xl px-4 py-6 pt-20">
    ...
    <div v-else class="space-y-5">
      ...
      <!-- Profile card shows name, quiz status and certificate download button -->
      <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div class="flex flex-wrap items-center gap-4">
          ...
          <div class="flex-1">
            ...
            <!-- Quiz badge changes colour depending on whether the student passed -->
            <div class="mt-2">
          ...
          <!-- Certificate button is disabled until every task is approved -->
          <div>
            <button
      ...
      <!-- Stats cards showing points, hours, tasks enrolled and quiz score -->
      ...
      <!-- Progress bar showing how many enrolled tasks have been approved -->
      <section v-if="assignments.length > 0" class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <!-- Certificate ready message only shown when every task is approved -->
      ...
      <!-- Tasks list with evidence submission forms -->
      <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 class="mb-4 text-lg font-bold text-gray-900">Your tasks</h2>

        <!-- Shown when the student has not enrolled in any tasks yet -->
        ...
        <div v-else class="space-y-3">
          <div v-for="item in assignmentItems" :key="item.assignmentId" class="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <div class="flex flex-wrap items-start justify-between gap-2">
              ...
              <!-- Status badge colour changes based on the current assignment status -->
              <span class="rounded-full border px-2.5 py-0.5 text-xs font-semibold" :class="statusClass(item.status)">{{ item.status }}</span>
            </div>

            <!-- Evidence form only shown when the task is In Progress -->
            <div v-if="item.status === 'In Progress'" class="mt-4 space-y-3 border-t border-gray-100 pt-4">
              <div>
                ...
                ></textarea>
                <!-- Checklist reminding the student what to include in their evidence -->
            ...
            <!-- Show rejection reason if the organisation rejected the evidence -->
            <div v-if="item.status === 'In Progress' && item.rejectionReason" class="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2">
              <p class="text-xs font-semibold uppercase tracking-wider text-red-400">Rejected — reason from organisation</p>
              <p class="mt-1 text-sm text-red-600">{{ item.rejectionReason }}</p>
              <p class="mt-1 text-xs text-red-400">Please update your evidence and resubmit.</p>
            </div>

            <!-- Show submitted evidence for tasks awaiting approval -->
      ...
      <!-- Recent activity feed built from quiz result and task status changes -->
      <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 class="mb-4 text-lg font-bold text-gray-900">Recent activity</h2>
        <div v-if="recentActivity.length === 0" class="text-sm text-gray-400">No activity yet — enroll in a task to get started.</div>
      ...
      <!-- Reminder that the AI chatbot is available in the bottom right corner -->
...
//Set a repeatable avatar colour based on the student's name
...
// Calculate the overall volunteering progress as a percentage
const progressPercent = computed(() =>
  !assignments.value.length ? 0 : Math.round((approvedCount.value / assignments.value.length) * 100)
)

// Certificate is only available when every enrolled task has been approved
const canDownloadCertificate = computed(() =>
  assignments.value.length > 0 && assignments.value.every(a => a.status === "Approved")
)

// Combine assignment records with task details so the tasks list can display everything
...
// Build a recent activity list from the student's quiz result and task statuses
...
// save the firestore listeners so they can be stopped when the page closes
let unsubscribeUser = null, unsubscribeAssignments = null, unsubscribeTasks = null, unsubscribePending = null

const subscribe = (uid) => {
  //Real time listener on the student's profile updates name, points and quiz score instantly
  ...
  // Real time listener on this student's assignments updates the task list instantly
  ...
  // Real time listener on all tasks this is needed to show task details alongside assignments
...
const submitEvidence = async (task) => {
  ...
  // Block submission if evidence is less than 20 characters
  if (text.length < 20) {
    alert("Please provide more detailed evidence (at least 20 characters).")
    return
  }

  // Block submission if no date reference is found in the evidence text
  ...
  try {
    // Firestore transaction safely checks the assignment status then saves the evidence
...
// Generate and download a PDF certificate using jsPDF and this runs entirely in the browser
...
// Check if a user is logged in when the page loads then start the Firebase listeners
onMounted(() => {
...
// Stop all Firebase listeners when the page closes
onUnmounted(() => {
  unsubscribeUser?.()
