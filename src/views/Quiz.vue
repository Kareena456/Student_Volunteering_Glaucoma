<!-- Quiz page wih 16 question glaucoma awareness with a 75% pass mark -->
<template>
  <div class="mx-auto max-w-5xl px-4 py-6 pt-20">
    ...
    <!-- Quiz questions ,unanswered questions are highlighted red on submit -->
    <div class="space-y-3">
      <div
    ...
    <!-- Passed message shown when student scores 75% or above -->
    ...
    <!-- Failed message shown when student scores below 75% -->
...
export default {
  setup() {
    ...
    // All 16 quiz questions with multiple choice and the correct answer index
    const questions = ref([
    ...
    // answers tracks what the student selected for each question 
    // score and correctCount are calculated when the student submits
    // passed,answeredCount and passingScore recalculate automatically when answers change
    ...
    // Calculate the score, save it to Firestore and show the result
    const submit = async () => {
      ...
      try {
        ...
        // Save the quiz score to the student's Firestore document so the Hubs page can check it
        await updateDoc(doc(db, "users", user.uid), { quizScore: score.value })
        resultReady.value = true
      } catch (err) { serverError.value = err.message }
    }

    // Reset everything so the student can retake the quiz from the beginning
    const retake = () => {
